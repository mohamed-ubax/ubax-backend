 ...
 *
 * <p><b>Identité :</b> l'identité principale est gérée par Keycloak (OAuth2/OIDC). Cette entité
 * duplique les informations essentielles pour les requêtes métier sans appel supplémentaire au
 * serveur Keycloak.
 *
 * <p><b>Rôles :</b> le rôle est synchronisé depuis le claim Keycloak à chaque login et dupliqué ici
 * pour les requêtes de filtrage (ex : lister les agents d'une agence). La vérification d'accès
 * réelle reste assurée par Spring Security.
 *
 * <p><b>Avatar :</b> stocké dans le bucket MinIO {@code users-avatars}. Seule l'URL est persistée
 * dans {@code avatarUrl}.
 *
 * <p><b>Soft delete :</b> un utilisateur n'est jamais supprimé physiquement (contraintes RGPD et
...
public class User extends BaseEntity {
   ...
   * false}). Sert de clé de réconciliation entre Keycloak et la base UBAX.
   */
  ...
  /**
   * Numéro de téléphone au format international (ex : {@code +221781234567}). Requis pour les
   * notifications SMS, la vérification OTP et le Mobile Money.
   */
  ...
  /**
   * {@code true} si une vérification KYC (pièce d'identité) a été effectuée. Obligatoire pour les
   * transactions dépassant un seuil défini (loi AML).
   */
