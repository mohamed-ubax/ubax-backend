 ...
 *
 * <p>Un ticket est créé par un locataire dans le cadre d'un contrat actif pour signaler un incident
 * (fuite, panne électrique, problème de serrure, etc.). Il transite par les statuts suivants
 * jusqu'à sa clôture :
 *
 * <pre>
 *  OPEN
 ...
 * </pre>
 *
 * <p><b>Notifications :</b> chaque transition de statut déclenche une notification au locataire via
 * le module {@code notification} (email + SMS).
 *
 * <p><b>Indicateurs de performance (SLA) :</b> le délai entre {@code OPEN} et {@code RESOLVED} est
 * mesuré pour alimenter les KPIs du tableau de bord SAV de l'agence.
 *
...
public enum TicketStatus {
   ...
   *
   * <p>Un agent SAV a pris en charge le ticket et est en train d'évaluer la nature et la gravité de
   * l'incident. Il peut poser des questions au locataire via la messagerie interne, consulter
   ...
   *
   * <p>Le technicien a effectué l'intervention et l'agent SAV a marqué le ticket comme résolu. Le
   * coût de réparation et la note de résolution sont renseignés. Le locataire est notifié et invité
   * à confirmer la résolution. Si aucune réponse dans le délai imparti, le ticket passe
   ...
   * ou lorsque l'agent SAV a constaté que l'incident n'est pas de la responsabilité de l'agence. Un
   * ticket {@code CANCELLED} n'est plus traité et ne génère pas de coût.
   */
  CANCELLED
}
