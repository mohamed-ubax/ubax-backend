 ...
 *
 * <p>Ce profil de préférences est utilisé pour :</p>
 * <ul>
 *   <li><b>Alertes automatiques</b> : notifier l'utilisateur dès qu'un bien
 *       correspond à ses critères (type, budget, ville, surface...).</li>
 *   <li><b>Recommandations personnalisées</b> : alimenter l'algorithme de
 *       suggestion de biens dans le feed et le dashboard.</li>
 *   <li><b>Personnalisation de l'interface</b> : langue, devise, fréquence
 *       des newsletters.</li>
 * </ul>
 *
 * <p><b>Relation :</b> 1-1 avec {@link User}. Une seule ligne par utilisateur,
 * créée automatiquement à l'inscription avec des valeurs par défaut.</p>
 *
 * <p><b>Tableaux PostgreSQL :</b> les champs {@code preferredCities} et
 * {@code amenities} utilisent le type natif PostgreSQL {@code TEXT[]} via
 * {@code hypersistence-utils} pour éviter une table de jointure inutile.</p>
 */
@Entity
@Table(
...
public class UserPreferences extends BaseEntity {
    ...
    /**
     * Utilisateur propriétaire de ces préférences.
     * Relation 1-1 : un seul enregistrement par utilisateur.
     */
    ...
    /**
     * Type de bien immobilier recherché par défaut.
    ...
    /**
     * Nature de la transaction souhaitée par défaut.
    ...
    /**
     * Liste des villes dans lesquelles l'utilisateur recherche un bien.
     * Type PostgreSQL natif {@code TEXT[]} via hypersistence-utils.
     * Requête : {@code WHERE 'Dakar' = ANY(preferred_cities)}
     */
    @Column(name = "preferred_cities", columnDefinition = "TEXT[]")
    private List<String> preferredCities;

    /**
     * Équipements indispensables pour l'utilisateur.
     * Type PostgreSQL natif {@code TEXT[]} via hypersistence-utils.
     * Valeurs possibles : {@code piscine | parking | gardiennage | climatisation |
    ...
    /**
     * État du bien accepté par l'utilisateur.
     * Valeurs : {@code NEW | GOOD | RENOVATE}
     * {@code null} = pas de préférence (tous états acceptés).
     */
    @Column(name = "preferred_condition", length = 20)
    private String preferredCondition;

    /**
     * Activer les alertes automatiques pour les nouvelles annonces
     * correspondant aux critères de recherche de l'utilisateur.
     */
    ...
    /**
     * Fréquence d'envoi des alertes de nouveaux biens.
     * Valeurs : {@code REALTIME | DAILY | WEEKLY}
     */
    ...
    /**
     * Recevoir les alertes et rappels par SMS.
     * Nécessite que {@code phone} soit renseigné et vérifié dans {@link User}.
     */
    ...
    /**
     * Recevoir les notifications push sur l'application mobile.
     * Nécessite que le token FCM soit enregistré côté backend.
     */
     ...
     * Défaut : {@code XOF} (Franc CFA). Valeurs supportées : {@code XOF | EUR | USD}
     */
    ...
    /**
     * Mode d'affichage par défaut des résultats de recherche.
     * Valeurs : {@code LIST | MAP | SPLIT}
     */
    @Enumerated(EnumType.STRING)
    ...
    /**
     * {@code true} si l'utilisateur a défini au moins un critère de recherche.
     * Utilisé pour savoir si les alertes peuvent être déclenchées.
     */
