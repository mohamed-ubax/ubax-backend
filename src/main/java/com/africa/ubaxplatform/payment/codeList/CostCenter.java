 ...
 *
 * <p>Détermine à quel niveau la dépense doit être imputée : à l'agence dans son ensemble
 * (charges de structure) ou à un bien immobilier précis (dépenses directement liées à la
 * gestion d'un actif). Ce découpage permet :
 *
 * <ul>
 ...
 * </ul>
 *
 * <p><b>Règle de validation :</b> si {@code costCenter = PROPERTY_SPECIFIC}, le champ
 * {@code propertyId} est <b>obligatoire</b> lors de la création d'une dépense. Le service
 * lève une {@code BadRequestException} si {@code propertyId} est absent dans ce cas.
 *
 * @see ExpenseCategory
...
public enum CostCenter {
   ...
   * Ces coûts sont absorbés par l'agence et n'apparaissent pas dans le compte d'exploitation
   * d'un bien précis.
   */
  AGENCY_GENERAL,
   ...
   * locataires, honoraires d'un diagnostiqueur immobilier, etc. Le bien concerné est identifié
   * par {@code propertyId} (obligatoire lorsque cette valeur est sélectionnée). Cette dépense est
