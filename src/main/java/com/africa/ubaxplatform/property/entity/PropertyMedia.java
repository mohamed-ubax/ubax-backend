 ...
 *
 * <p><b>Stockage :</b> fichiers hébergés dans le bucket MinIO {@code properties-media}. Seule l'URL
 * est persistée. Les vidéos lourdes peuvent pointer vers un CDN externe.
 *
 * <p><b>Cycle d'upload :</b>
 *
 * <ol>
 *   <li>Frontend demande URL présignée PUT : {@code GET
 *       /api/storage/presign?bucket=properties-media&key={propertyId}/{uuid}.webp}
 *   <li>Upload direct vers MinIO via l'URL présignée.
 *   <li>Frontend notifie le backend avec l'URL publique finale.
 *   <li>Backend crée l'enregistrement {@code PropertyMedia}.
 * </ol>
 *
 * <p><b>Photo de couverture :</b> une seule entrée par bien peut avoir {@code cover = true}. C'est
...
public class PropertyMedia extends BaseEntity {
   ...
   * être un lien externe (Matterport, etc.). Format : {@code
   * http://minio:9000/properties-media/{propertyId}/{uuid}.webp}
   */
  ...
  /**
   * Type MIME du fichier, détecté au moment de l'upload. Exemples : {@code image/webp}, {@code
   * video/mp4}, {@code application/pdf}
   */
  @Column(name = "mime_type", length = 100)
  private String mimeType;

  /**
   * {@code true} si ce média est la photo de couverture de l'annonce. Une seule entrée par bien
   * devrait avoir {@code cover = true}.
   */
  ...
  /**
   * Position d'affichage dans le carrousel (ordre croissant). Modifiable via drag-and-drop dans
   * l'interface de gestion des médias.
   */
